/**
 * Blockaway Web Proxy Accessor Module
 * Automates accessing www.cdn.blockaway.net and submitting search queries
 */
import { chromium } from '@playwright/test';
export class BlockawayAccessor {
    constructor() {
        this.browser = null;
        this.page = null;
    }
    /**
     * Initialize the browser and navigate to cdn.blockaway.net
     */
    async initialize() {
        try {
    ...
    /**
     * Search for a website through Blockaway webproxy
    ...
    async searchWebsite(url) {
        ...
        try {
            ...
            if (!await searchInput.isVisible()) {
                throw new Error('✗ Search input box not found on cdn.blockaway.net');
            }
            ...
            const goButton = await this.page.locator('button:has-text("Go"), button:has-text("GO"), button[type="submit"], input[type="submit"], button:has-text("Browse"), button:has-text("Access")').first();
            if (!await goButton.isVisible()) {
                throw new Error('✗ Go button not found on cdn.blockaway.net');
            }
...
/**
 * Standalone function to run the Blockaway access workflow
 * @param targetUrl The target URL to access through Blockaway
 * @returns The BlockawayAccessor instance for further interaction
 */
export async function runBlockawayAccess(targetUrl = 'www.stockscanner.net') {
    const accessor = new BlockawayAccessor();
    try {
        console.log(`Initializing browser and navigating to cdn.blockaway.net...`);
        ...
        console.error('Error accessing Blockaway:', error);
