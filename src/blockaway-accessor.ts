  ...
  async searchWebsite(url: string): Promise<string> {
    ...
    try {
      ...
      const goButton = await this.page.locator(
        'button:has-text("Go"), button:has-text("GO"), button[type="submit"], input[type="submit"], button:has-text("Browse"), button:has-text("Access")'
      ).first();
 ...
 */
export async function runBlockawayAccess(targetUrl: string = 'www.stockscanner.net'): Promise<BlockawayAccessor> {
