...
import { launchBrowserWithProxy, launchBrowserWithProxyium, launchBrowserWithCroxyProxy, launchBrowserWithVPNBook, launchBrowserWithBlockaway } from './src/browser-factory';
// Global map to track proxy statistics across test runs
const proxyStatsMap = new Map();
// Helper to get proxy key
function getProxyKey(proxy) {
    return `${proxy.host}:${proxy.port}`;
}
// Helper to update proxy stats
function updateProxyStats(proxy, success, failureReason) {
...
function getUniqueUserAgent(index) {
...
async function humanScroll(page) {
    ...
    if (Math.random() < 0.7) {
        ...
        while (currentScroll > scrollUpTarget) {
            ...
            currentScroll -= step;
            if (currentScroll < scrollUpTarget)
                currentScroll = scrollUpTarget;
            await page.evaluate((y) => window.scrollTo(0, y), currentScroll);
...
test.describe('StockScanner Multi-Location Health Check', () => {
    ...
    test.beforeAll(async () => {
        ...
        try {
            ...
            const nonUSProxies = validatedProxies.filter(p => p.validated &&
                p.verified &&
                p.realCountry &&
    ...
    test('Visit and scroll all pages from multiple locations', async () => {
        ...
        const problematicProxies = allProxyStats.filter(stats => stats.successRate < 50 || stats.successes === 0);
        ...
        if (directLocationFailed) {
            throw new Error(`Health check failed for direct GitHub connection. This indicates the website is down or unreachable.`);
        }
